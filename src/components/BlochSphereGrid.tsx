...
function BlochSphere({ data, size = 0.8 }: { data: BlochSphereData; size?: number }) {
  ...
  const mainColor = isX ? '#00f0ff' : '#ff00aa';
  ...
  if (activeSpheres.length === 0) {
    return (
      <div className="glass-panel p-3 text-center">
        <p className="font-display text-xs neon-text-cyan opacity-50 tracking-widest">
          BLOCH SPHERES
        </p>
        ...
        {activeSpheres.map(s => {
          ...
          return (
            <div
              ...
              style={{ color: isX ? '#00f0ff' : '#ff00aa' }}
            >
              <span
                className="inline-block w-2 h-2 rounded-full"
                style={{ background: isX ? '#00f0ff' : '#ff00aa', boxShadow: `0 0 4px ${isX ? '#00f0ff' : '#ff00aa'}` }}
              />
